...
# Extract num_images from the end of the list
	...
	try:
		...
		# Convert GeneratedImage objects to PIL Image objects
		pil_images = [image.image for image in images]
...
# Extract num_images from the end of the list
	...
	try:
		...
		if steps is None:
			steps = 2 if "schnell" in model else 14
...
# Extract num_images from the end of the list
	...
	if not steps or steps.strip() == "":
		base_model = model.replace("-4-bit", "").replace("-8-bit", "")
		steps = 2 if "schnell" in base_model else 20
	else:
		steps = int(steps)
